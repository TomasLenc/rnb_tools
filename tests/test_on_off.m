...
function test_get_on_off_rectify(test_case)
   ...
   [~, on_val] = get_on_off(x, fs, 0.200, ...
        'pulse_period_sec', 0.8 ...
        );
    
   [~, on_val_half_rectif] = get_on_off(x, fs, 0.200, ...
        'pulse_period_sec', 0.8, ...
        'half_rectify', true ...
        );

    [~, on_val_full_rectif] = get_on_off(x, fs, 0.200, ...
        'pulse_period_sec', 0.8, ...
        'full_rectify', true ...
        );


    assert(all(on_val_half_rectif) >= 0)
    assert(all(on_val_full_rectif) > 0)
    
    assert(on_val_half_rectif > on_val)
    assert(on_val_full_rectif > on_val_half_rectif)
